...
| `openInApp.test.ts` | `openInSlicer()` with an explicitly configured command (never the OS-default auto-detect path, which could pop a real GUI): spawns the configured executable with the model path as its sole argument (verified end-to-end via a marker file written by a throwaway Node script), resolves `ok: false` with an error message when the command doesn't exist, and honours `PRINTSORT_SLICER_COMMAND` over the persisted config value. |
...
| `config.test.ts` | Env-derived roots (`PRINTSORT_ROOTS` / `PRINTSORT_MODELS_DIR`), managed-root dedupe/merge/non-persistence, and `slicerCommand` (default empty, trimmed round-trip through `saveConfig`, `PRINTSORT_SLICER_COMMAND` override). |
...
- `openInApp.ts`'s auto-detect path (locating a Bambu Studio install, or the OS-default `start`/`open`/`xdg-open` fallback) is deliberately never invoked from a test — it would launch a real process/GUI. Only the explicitly-configured-command path is covered.
