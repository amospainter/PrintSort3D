...
| `src/paths.ts` | Resolves and creates the `assets/` cache directory (every per-file artifact — thumbnail, embedded WebPs, baked mesh — lives under `assets/<fileId>/`). |
| `src/scanner.ts` | Walks each configured root looking for `.stl` / `.3mf` / `.obj` / `.zip` / `.f3d` files, upserts rows into `files`, and flags rows that disappeared as `missing` instead of deleting them (a scan job model with progress / cancel / `reprocess-stale` modes). The per-file work is split into `scanArtifacts.ts` (pure — parse, hash, bake, render, no DB; optionally run in `worker_threads` via `scanPool.ts` when `SCAN_WORKERS` is set) and the DB write-back here. STL/OBJ/3MF get a baked render mesh + dimensions + geometry hash; `.3mf` also gets `threeMf.ts` metadata + `sliceInfo.ts` estimates; `.f3d` gets only its embedded Fusion preview via `f3d.ts`; `.zip` gets an entry count. |
...
| `src/assets.ts` | Owns everything under `assets/<fileId>/`. `cacheThreeMfImages(fileId, filePath)` — calls `listThreeMfImages`, converts each to WebP via `sharp`, writes `<name>.webp`, returns the saved filenames (best-effort: one bad image is skipped, not fatal). `saveThumbnail(fileId, buffer)` / `thumbnailFilePath(fileId)` — the `thumbnail.png` for a file, written by both the 3MF scanner and the client upload route. `cacheBakedMesh(...)` — the `mesh.bin.gz`. `deleteCachedImages(fileId)` removes the whole directory — called when a root is removed. |
...
### How thumbnails are generated (all server-side)

A `.3mf` file is just a zip, so a Bambu Lab file's embedded plate render is pulled out directly with no rendering. `.stl`, `.obj`, and non-Bambu `.3mf` exports embed nothing, so the scanner **rasterizes them on the CPU** — `server/src/thumbnail.ts` `renderBakedThumbnail` is a small software rasterizer (z-buffer, flat two-sided Lambert shading, rendered at 2× and downscaled by `sharp` for antialiasing) that draws the already-baked mesh (`meshBake.ts` → `BakedPart[]`) straight into a pixel buffer.

There is deliberately **no WebGL / three.js / GPU on the server** — headless GL (`headless-gl` / `gl`) needs native bindings, and the server frequently runs in a minimal Docker container with no display or GPU at all. `scanner.ts` bakes each model once and feeds the same parts to both the mesh cache and the thumbnail renderer; the render is skipped when the file brought its own embedded thumbnail. An earlier version rendered these in the browser (`ThumbnailGenerator.tsx`, now deleted) and POSTed them back — that made STL-heavy result pages slow (a full page refetch per finished thumbnail) and didn't work at all for anyone browsing the catalogue on a machine without WebGL.
...
| `src/pages/Library.tsx` | Grid/table toggle, search box, tag/extension/`root`/`folder` filters, multi-select (click / shift-range / rubber-band drag) with a bulk-tag toolbar, and pagination (page-size selector: 10/15/20/30/40/50, default 20 — passed to `GET /api/files` as `page`/`pageSize`; changing any filter or the page size resets to page 1). All filter state lives in the URL. Thumbnails are all server-rendered now, so this just displays `file.thumbnailUrl`. |
...
2. User clicks "Rescan now" → `POST /api/scan` → `runScan()` walks every configured root recursively, filtering to `.stl`/`.3mf`/`.obj`/`.zip`/`.f3d`.
3. For each file: if it's new, or its `mtime` changed, or its `scanner_version` is behind the code's current version (see below), (re)compute and store bounding-box dimensions (`dimensions.ts`), the baked render mesh (`meshBake.ts` → `assets.ts`), content/geometry hashes (`fingerprint.ts`), and — for `.3mf` — embedded thumbnail + metadata + per-image WebPs (`threeMf.ts` / `assets.ts`). Unless the file supplied its own thumbnail, a PNG is rasterized on the CPU from the baked mesh (`thumbnail.ts`) and cached at `server/assets/<id>/thumbnail.png`.
4. Any previously-known file not seen this pass gets `missing = 1`.
5. Client reloads `GET /api/files` and renders the grid straight from `file.thumbnailUrl` — nothing is generated client-side.

## Testability decisions

`db.ts`, `config.ts`, and `paths.ts` all read their file locations from environment variables (`DB_PATH`, `CONFIG_PATH`, `ASSETS_DIR`) with the normal on-disk locations as defaults. Tests set these to a fresh temp directory and dynamically `import()` the modules afterward (static imports are hoisted and would run before the env vars are set), giving each test file a fully isolated database/config/asset-cache with no shared state between test files. See [testing.md](testing.md).
