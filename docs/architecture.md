...
5. Client reloads `GET /api/files`, renders the grid. Files still missing a `thumbnailUrl` (STL/OBJ, or a 3MF with no embedded thumbnail) each mount a `ThumbnailGenerator`, which renders and uploads a PNG, which is cached at `server/assets/<id>/thumbnail.png` and referenced from then on.
